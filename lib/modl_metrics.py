...
class MODLMetrics:
    ...
    def iou(self, x_true, y_true, w_true, h_true, x_pred, y_pred, w_pred, h_pred, t, pred_confid_tf):
        ...
        ow = self.overlap(x + xoffset, w * 4*256., x_true + xoffset, w_true * 4*256.)
        oh = self.overlap(y + yoffset, h * 4*160., y_true + yoffset, h_true * 4*160.)
        ...
        fid_t = K.greater(pred_confid_tf, 0.5)
    ...
    def recall(self, y_true, y_pred):
        ...
        pred_conf_tensor_reduc = K.mean(pred_conf_tensor, axis=-1)
        ...
        ave_iou, recall, precision, obj_count, intersection, union, ow, oh, x, y, w, h = self.iou(truth_xy_tensor[:, :, 0],
                                                                                            truth_xy_tensor[:, :, 1],
                                                                                            truth_wh_tensor[:, :, 0],
                                                                                            truth_wh_tensor[:, :, 1],
                                                                                            pred_xy_tensor[:, :, 0],
