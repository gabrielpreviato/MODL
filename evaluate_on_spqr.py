...
for test_dir in test_dirs:
    ...
    for rgb_path, obs_path in zip(rgb_paths, obs_paths):
        ...
        # if len([a for a in obs if a[2] == 0]) < 2:
        #     continue
        total_images += 1
        ...
        #
        #     for i in range(len(gt_obj_str)):
        #         class_str = 'robot' if gt_obj_str[i] == 'nao' else gt_obj_str[i]
        #         w = gt_obj_coords[i][2]
        #         h = gt_obj_coords[i][3]
        #         x_c = gt_obj_coords[i][0] + w/2
        #         y_c = gt_obj_coords[i][1] + h/2
        #         writer.writerow([int(x_c / 32), int(y_c / 32), (x_c % 32) / 32, (y_c % 32) / 32, w, h, 0, 0, class_str])
        ...
        if showImages:
            if results[1] is not None:
                if model_name == 'modl':
                    EvaluationUtils.show_detections_multiclass(rgb, results[1], gt=gt_obs, save=True, save_dir="save_"+str(number_classes)+"_real_notransf",
            ...
            if results[0] is not None:
                EvaluationUtils.show_depth(rgb, results[0], save=True, save_dir="save_"+str(number_classes)+"_real_notransf", file_name="sav_"+ str(i) +".png", sleep_for=10)
...
if confMatrix:
    ...
    if number_classes == 2:
        classes = ["nothing", "ball", "robot"]
    elif number_classes == 3:
        classes = ["nothing", "goal", "ball", "robot"]
    elif number_classes == 3:
        classes = ["nothing", "goal", "ball", "robot_team", "robot_opponent"]

    plot_confusion_matrix(conf_mat, classes=classes,
                          normalize=False, title=' ')

    plt.savefig('confusion_matrix_' + str(number_classes) + '_real_notransf.png', bbox_inches='tight')
